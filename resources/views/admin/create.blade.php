<x-layout>
    <header class="w-full h-fit py-3 px-5">
        <div class="flex justify-between items-center bg-white py-1 px-3 sm:px-9 rounded-md">
            <h4 class="text-xl font-medium">Create New Post</h4>
    ...
    <section class="container mx-auto my-3 px-5 sm:px-0">
        <form action= "/admin/posts/" method="POST" class="w-full">
            @csrf

            <div class="my-5">
                <label for="title" class="text-xl block mb-2 ml-1 font-bold">Title</label>
                <input type="text" value="{{ old('title') }}" name="title" id="title" class="px-3 py-2 text-lg bg-transparent font-medium w-full rounded-md border-2 shadow-base block border-gray-400 text-gray-400 focus:border-green-500 focus:text-gray-700" placeholder="Enter posts title">
                @error('title')
            ...
            <div class="my-5">
                ...
                <input type="text" value="{{ old('describtion') }}" name="describtion" id="describtion" class="px-3 py-2 text-lg bg-transparent font-medium w-full rounded-md border-2 shadow-base block border-gray-400 text-gray-400 focus:border-green-500 focus:text-gray-700" placeholder="Enter posts describtion">
                @error('describtion')
                    <span class="text-red-600 text-sm my-1 block">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="my-5">
                ...
                <input type="text" value="{{ old('thumbnail') }}" name="thumbnail" id="thumbnail" class="px-3 py-2 text-lg bg-transparent font-medium w-full rounded-md border-2 shadow-base block border-gray-400 text-gray-400 focus:border-green-500 focus:text-gray-700" placeholder="Enter posts thumbnails url">
                @error('thumbnail')
                    <span class="text-red-600 text-sm my-1 block">{{ $message }}</span>
                @enderror
            </div>
